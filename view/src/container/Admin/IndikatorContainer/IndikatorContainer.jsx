...
class IndikatorContainer extends React.Component{
    ...
    componentDidMount(){
        ...
        axios.get("/admin/competency/specific/all",{
        },
            ...
            new Promise((resolve, reject)=>{
                ...
                for(let i =0 ; i < competencies.length; ++i){
                    _datas.push({
                        ...
                        base    : competencies[i].baseCompetency.name,
                        base_id : competencies[i].baseCompetency.id,
    ...
    onEditHandler = () => {
                ...
                for( let  j = 0 ; j < topics.length ; ++j){
                    if(topics[j].id === this.state.selectedRows[0].topic_id){
                        ...
                        for ( let k = 0 ; k < cores.length; ++ k){
                            if(cores[k].id === this.state.selectedRows[0].core_id){
        ...
        this.setState({
            ...
            value : {
                ...
                core        : {value : this.state.selectedRows[0].core_id , label :this.state.selectedRows[0].core, bases : bases},
                base        : {value : this.state.selectedRows[0].base_id , label : this.state.selectedRows[0].base}
            }
        }, () =>{
    ...
    onSubmithandler = ()=>{
        if( (this.state.value.subject === null)){
        ...
        else if(this.state.value.base === null){
            swalinstance.fire({title:"Wajib memilih Kompetensi Dasar!", icon:"error"})
            return
        }else if(this.state.value.description === "" || this.state.value.description === null){
            swalinstance.fire({title:"Indikator tidak boleh kosong!", icon:"error"})
            return
        }else{
            this.setState({
                addsubmitting:true
            ...
            value.base_id = value.base.value
            
            axios.defaults.headers.common['Authorization'] = `Bearer ${this.props.fakeAuth.data.user.token}` 
            axios.post("/admin/competency/specific/store",{
                value:value
            },
            {withCredentials:true}).then((res)=>{
                this.setState({
    ...
    onUpdateHandler = () =>{
        if( (this.state.value.subject === null)){
            ...
            value.base_id = value.base.value
            axios.defaults.headers.common['Authorization'] = `Bearer ${this.props.fakeAuth.data.user.token}` 
            axios.post("/admin/competency/specific/update",{
                value:value
            },
            {withCredentials:true}).then((res)=>{
                this.setState({
    ...
    onDeleteHandler = () =>{
        swalinstance.fire({
            ...
            if(click.isConfirmed){
                ...
                axios.defaults.headers.common['Authorization'] = `Bearer ${this.props.fakeAuth.data.user.token}` 
                axios.post("/admin/competency/specific/destroy",{
                    value_ids:value_ids
                },
                {withCredentials:true}).then((res)=>{
                    if(res.data.status){
    ...
    render(){
        const home = <div className="admin-content-container">
                        <div className="admin-content-header">
                            <div className="admin-title">
                                <h2 style={{ marginBottom:"0" }}>Indikator</h2>
                                <small>
                                    <span>Admin</span> <span> / </span> <span> Indikator</span>
                                </small>
                            </div>
                        ...
                        <div className="admin-content-body">
                            <div className="admin-content-body data">
                                    {this.state.loading?  <span> <span className="fas fa-spinner spinning"></span> Loading...</span>:<TableComponent setSearch={this.setSearch} onSelectedRowsHandler={this.onSelectedRowsHandler} data={this.state._display} cols={columns} title="Daftar Indikator"/>}
                            </div>
                        </div>
                    </div>
        ...
        const body = <span>
            ...
            <div className="form-group row">
                <div className="col-3">
                    <label htmlFor="" className="control-label required">Indikator</label>
                </div>
                <div className="col-9">
        ...
        return(
            <span>
                ...
                <ModalComponent title={(this.state.overrideadd === true)?"Ubah Indikator":"Tambah Indikator"} body={body} footer={footer} open={this.state.modaladd?true:false} onAddHideHandler={this.onAddHideHandler}></ModalComponent>
            </span>
        )
    }
}

export default IndikatorContainer